...
namespace Verifone.ECRTerminal
{
    ...
    public partial class ECRTerminalManager : IECRTerminalManager
    {
        ...
        public string EncodingName => ECRProtocol.EncodingName;
        ...
        /// <summary>
        /// Masks a string by replacing characters up to <paramref name="maxLength"/> with the given mask character.
        /// </summary>
        ...
        private string GetBonusStatusCodeFromPhase(string statusPhase)
        {
            string result = "2"; //default to chip

            if (statusPhase == "1")
                result = "2";
            else if (statusPhase == "3")
                result = "1";
            else if (statusPhase == "6")
                result = "4";
            else if (statusPhase == "9")
                result = "3";
        ...
        private void HandleBonusCardFound(TransactionStatusEventArgs e)
        {
            ...
            string customerNumber = e.Info?.Trim() ?? StringResources.NA;
            string memberClass = StringResources.NA;
            string statusCode = GetBonusStatusCodeFromPhase(e.StatusPhase);
            string statusMessage = StringResources.GetTransactionStatusResultCodeMessage(e.StatusResultCode) ?? StringResources.NA;
            PaymentSession session = GetLastRunningSession() as PaymentSession;

            if (session != null)
            {
                session.UpdateBonusInfo(customerNumber, memberClass, statusCode, statusMessage);
        ...
        /// <summary>
        /// Performs post-processing for a bonus-card-only event after default handling.
        /// </summary>
        /// <param name="customerNumber">The detected customer/bonus number.</param>
        /// <param name="memberClass">The detected membership class.</param>
        /// <param name="statusCode">The terminal status/result code.</param>
        /// <param name="statusMessage">The terminal status message.</param>
        /// <param name="e">The original transaction status event.</param>
        /// <remarks>
        /// The base implementation performs these actions:
        /// <list type="bullet">
        /// <item><description>Disables bonus card mode after a short delay.</description></item>
        /// <item><description>Raises <see cref="BonusResultReceived"/> with the parsed bonus details.</description></item>
        /// <item><description>Raises <see cref="TransactionTerminalAbortReceived"/> for the current transaction.</description></item>
        /// </list>
        /// Override to add custom behavior; call the base method unless replacing the default flow.
        /// </remarks>
        protected virtual void HandleBonusCardOnlyAfter(string customerNumber, string memberClass, string statusCode, string statusMessage, TransactionStatusEventArgs e)
        {
            RunAction(DisableBonusCardMode, 500);
            OnCustomerRequestResultReceived(new CustomerRequestResultEventArgs(customerNumber, memberClass, statusCode, statusMessage));
        ...
        private void HandleBonusCardOnly(TransactionStatusEventArgs e)
        {
            ...
            HandleBonusCardOnlyAfter(customerNumber, memberClass, statusCode, statusMessage, e);
        }
        ...
        /// <summary>
        /// Called when a protocol-level error is detected, before the error is propagated
        /// to <see cref="TerminalError"/> subscribers.
        /// <para>
        /// If the active session is still running, it is marked as failed with the
        /// reported exception. If the underlying cause is an <see cref="IOException"/>,
        /// <see cref="Disconnect"/> is called to release the port, allowing future
        /// operations to re-establish communication automatically.
        /// </para>
        /// <para>
        /// Override to add custom recovery or reconnection strategies; call the base
        /// method to retain default error and disconnect handling.
        /// </para>
        /// </summary>
        ...
        {
            Trace.WriteLine($"{nameof(HandleProtocolError)}:{e.Exception.GetType().Name}", GetType().FullName);

            if (LastSession?.IsActive == true)
                SetLastSessionError(e.Exception);

            if (ContainsIOException(e.Exception))
                Disconnect();
        }

        /// <summary>
        /// Returns <c>true</c> if <paramref name="exception"/> is or wraps an <see cref="IOException"/>,
        /// by walking the <see cref="Exception.InnerException"/> chain and flattening any
        /// <see cref="AggregateException"/> encountered along the way.
        /// </summary>
        /// <param name="exception">The exception to inspect.</param>
        private static bool ContainsIOException(Exception exception)
        {
                        ...
                        && session.State == SessionState.Running)
                    {
                        ...
                        if (session1?.IsBonusHandled == true)
                        {
                                ...
                                && session1.State == SessionState.BonusDetectedAndHalted)
                            {
                                ...
                                bonusInfo = new CustomerRequestResultEventArgs(session1.BonusCustomerNumber, session1.BonusMemberClass, session1.BonusStatusCode, session1.BonusStatusText);
                            }
                        }
                    }
                }
        ...
        private void OnCustomerRequestResult(object sender, CustomerRequestResultEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    session.UpdateBonusInfo(e.CustomerNumber, e.MemberClass, e.StatusCode, e.StatusText);
                }
                else
                {
    ...
    public abstract class TerminalSession
    {
        ...
        /// <summary>
        /// Holds the most recent transaction status event from the terminal.
        ...
        protected TerminalSession(string sessionId)
        {
            ...
            SessionId = sessionId?.Trim() ?? StringResources.NA;
    ...
    public class RetrieveSession : TerminalSession
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="RetrieveSession"/> class for retrieving
        /// a transaction result from the terminal.
        /// </summary>
        /// <param name="transactionId">
        /// The transaction identifier to retrieve. Use <see cref="ECRProtocol.EmptyTransactionId"/>
        /// or <c>default(string)</c> to request the most recent transaction when supported by the terminal.
        /// </param>
        /// <param name="timestamp">
        /// The timestamp of the transaction to retrieve. Use <c>default(DateTime)</c> (or <see cref="DateTime.MinValue"/>)
        /// to indicate that the most recent transaction should be returned, if supported.
        /// </param>
