...
namespace Verifone.ECRTerminal
{
    internal partial class ECRProtocol : IDisposable
    {
        ...
        internal static string EncodingName { get; } = "ISO-8859-15";
        ...
        private int _readerRunning; // 0 = false, 1 = true; accessed via Interlocked
        ...
        {
            Interlocked.Exchange(ref _readerRunning, 0);
            try
            {
        ...
        private void EnsurePortActive()
        {
            ...
            if (Interlocked.CompareExchange(ref _readerRunning, 0, 0) == 0)
                StartReaderThread();
        }
        ...
        private void OnCustomerRequestResultReceived(string customerNumber, string memberClass, string statusCode, string statusText)
        {
            EventHandler<CustomerRequestResultEventArgs> h = CustomerRequestResult;
            h?.Invoke(this, new CustomerRequestResultEventArgs(customerNumber, memberClass, statusCode, statusText));
        }
        ...
        private void HandleCustomerRequestResult(byte[] data)
        {
            ...
            string msg = _encoding.GetString(data);
            string statusCode = msg.Substring(1, 1);
            string customerNumber = msg.Substring(2, 20).Trim();
            string memberClass = msg.Substring(22, 2);
            string statusText = StringResources.GetCustomerBonusStatusMessage(statusCode);

            Trace.WriteLine($"{nameof(HandleCustomerRequestResult)}:{customerNumber} {memberClass} {statusCode} {statusText}", GetType().FullName);

            OnCustomerRequestResultReceived(customerNumber, memberClass, statusCode, statusText);
        }
        ...
        private void StartReaderThread()
        {
            ...
            // Atomically transition _readerRunning from false (0) to true (1).
            // If another caller already set it, CompareExchange returns 1 → bail out.
            if (Interlocked.CompareExchange(ref _readerRunning, 1, 0) != 0)
                return;
        ...
        // Serializes all Send* callers so only one frame is in-flight at a time.
        // Rejects concurrent callers immediately: if a send is already in progress,
        // the new command is dropped and a ProtocolError event is raised.
        private readonly SemaphoreSlim _sendLock = new SemaphoreSlim(1, 1);

        /// <summary>
        /// Attempts to send a framed protocol message to the terminal.
        /// If a send is already in progress, the call is rejected immediately rather than queued:
        /// <see cref="OnProtocolError"/> is raised with an <see cref="InvalidOperationException"/>
        /// and the method returns without writing to the port.
        /// Otherwise, the message is forwarded to <see cref="SendWithRetry"/>, and any failure
        /// is reported through <see cref="OnProtocolError"/>.
        /// </summary>
        /// <param name="request">The fully framed message bytes to send (including STX, ETX, LRC).</param>
        /// <param name="commandId">The command identifier used for event correlation on ACK.</param>
        private void SendOrReportError(byte[] request, string commandId)
        {
            if (!_sendLock.Wait(0))
            {
                Trace.WriteLine($"{nameof(SendOrReportError)}: Busy, rejecting command '{commandId}'", GetType().FullName);
                OnProtocolError(new InvalidOperationException(
                    StringResources.GetCommonString(StringResources.MessageTerminalCommunicationError)));
                return;
            }
            try
            {
                if (!SendWithRetry(request, commandId, out Exception error))
                    OnProtocolError(error);
            }
            finally
            {
        ...
        /// sets <paramref name="error"/> to the encountered exception or a timeout exception.
        /// </para>
        /// </summary>
        ...
        /// <param name="error">
        /// When the method returns <c>false</c>, contains the exception that caused the failure,
        /// or a <see cref="TimeoutException"/> if all retry attempts failed.
        /// When the method returns <c>true</c>, <paramref name="error"/> is <c>null</c>.
        /// </param>
        ...
        /// </returns>
        private bool SendWithRetry(byte[] message, string commandId, out Exception error, int maxRetries = 3, int timeoutMs = 3000)
        {
        ...
        internal void SendHandshake()
        {
            ...
            SendOrReportError(request, CommandId.TestTerminal);
        }
        ...
        private void SendRunPayment(decimal amount, string transactionId, bool bonusHandled, string authorizationCode)
        {
            ...
            SendOrReportError(request, CommandId.PurchaseTransaction);
        }
        ...
        private void SendSimpleTransaction(string type, decimal amount, string commandId)
        {
            ...
            SendOrReportError(request, commandId);
        }
        ...
        internal void SendAuxiliaryAcceptMode(bool enable)
        {
            ...
            SendOrReportError(request, enable ? CommandId.EnableAuxiliaryMode : CommandId.DisableAuxiliaryMode);
        }
        ...
        internal void SendTransactionAbort()
        {
            ...
            SendOrReportError(request, CommandId.AbortTransaction);
        }
        ...
        internal void SendTransactionAccept(string transactionId, bool accept)
        {
            ...
            SendOrReportError(request, accept ? CommandId.AcceptTransaction : CommandId.RejectTransaction);
        }
        ...
        internal void SendCustomerRequest(bool stopActive)
        {
            ...
            SendOrReportError(request, CommandId.RequestBonusCardInfo);
        }
        ...
        internal void SendRequestStatus()
        {
            ...
            SendOrReportError(request, CommandId.RequestTerminalStatus);
        }
        ...
        internal void SendTCSMessageRequest()
        {
            ...
            SendOrReportError(request, CommandId.RetrieveTCSMessage);
        }
        ...
        internal void SendTerminalVersionRequest()
        {
            ...
            SendOrReportError(request, CommandId.RequestTerminalVersion);
        }
        ...
        internal void SendDisplayText(string line1, string line2, bool bigFont)
        {
            ...
            SendOrReportError(request, CommandId.DisplayText);
        }
        ...
        internal void SendClearDisplay()
        {
            ...
            SendOrReportError(request, CommandId.ClearDisplayText);
        }
    }
}