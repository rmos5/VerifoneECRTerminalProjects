    ...
    internal partial class ECRProtocol : IDisposable
    {
        ...
        private void DisposeLocalResources()
        {
            ...
            try
            {
                if (IsOpen)
                    _port.Close();
                //give some time to release underlying buffers
                Thread.Sleep(500);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Formats a monetary amount to a 12-digit numeric string without a decimal separator.
        /// </summary>
        /// <param name="value">The amount to format, expressed in currency units (e.g., euros).</param>
        /// <returns>
        /// The amount multiplied by 100 and converted to a zero-padded 12-digit string (e.g., <c>12.34 → "0000000001234"</c>).
        /// </returns>
        /// <remarks>
        /// This format is used by Verifone ECR messages (see <c>TransactionRequestEx2</c>),
        /// where amounts are represented as integer values of cents with fixed 12-character width.
        /// </remarks>
        /// <summary>
        /// Converts a byte array into a continuous uppercase hexadecimal string representation.
        /// </summary>
        /// <param name="message">The raw message bytes to format.</param>
        /// <returns>
        /// A hexadecimal string where each byte is represented by two uppercase characters (e.g., <c>"023FA1"</c>).
        /// </returns>
        /// <remarks>
        /// This method is primarily used for diagnostic logging and tracing of ECR protocol communication,
        /// providing a readable representation of binary message data.
        /// </remarks>
                    ...
                    while (assembling)
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (nextByte == ETX)
                            {
                                ...
                                partData.RemoveAt(partData.Count - 1);
                                break;
                            }
                        }
